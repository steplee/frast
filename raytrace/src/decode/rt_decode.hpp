...
inline bool decode_node_to_tile(
	...
	for (int i=0; i<n_mesh; i++) {
		...
		if (forceTriList) {
			...
			md.ind_buffer_cpu.clear();
			for (int i=0; i<oldInds.size()-2; i++) {
				// TODO: Check if this is correct, and uncomment if so
				// Skip degenerate tris
				if (oldInds[i+0] == oldInds[i+1] or oldInds[i+0] == oldInds[i+2] or oldInds[i+1] == oldInds[i+2]) continue;
				if (i % 2 == 0) {
