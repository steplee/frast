...
struct WindowX {
	...
	inline WindowX(const std::string &name, int w, int h, int c) : name(name), windowWidth(w), windowHeight(h), channels(c) {}
	...
	inline void createWindow() {
		...
		xcb_create_pixmap(xcbConn, depth, rect, screen->root, windowWidth, windowHeight);
		// xcbImg = xcb_image_get(xcbConn, rect, 0,0, windowWidth, windowHeight, 0xff'ff'ff'ff, XCB_IMAGE_FORMAT_XY_PIXMAP);
		// xcb_rectangle_t     rect_coord = { 0, 0, (uint16_t)windowWidth, (uint16_t)windowHeight};
		// xcb_poly_fill_rectangle(xcbConn, rect, bgcolor, 1, &rect_coord);

		xcb_flush (xcbConn);

		xcb_change_property(xcbConn,
				XCB_PROP_MODE_REPLACE, xcbWindow,
				XCB_ATOM_WM_NAME, XCB_ATOM_STRING,
				8, name.length(), name.c_str());
	...
	inline void update(const Image& img) {
		...
		auto image = xcb_image_create_native(xcbConn,w,h,XCB_IMAGE_FORMAT_Z_PIXMAP,depth,tmp,w*h*4,tmp);
		...
		while ((e = xcb_poll_for_event (xcbConn))) {
			switch (e->response_type & ~0x80) {
				...
				case XCB_KEY_RELEASE: {
										...
										if (key0 == XK_p) is_paused = not is_paused;
										if (key0 == XK_q) is_quit = true;
...
void imshow(const std::string &name, const Image& img) {
	...
	if (windows.find(name) == windows.end()) {
		windows[name] = std::make_unique<WindowX>(name, img.w, img.h, img.channels());
