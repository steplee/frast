...
struct Image {
  ...
  inline void free() {
	...
	void copyFrom(const Image& other) {
		// if (other.buffer and buffer and ownBuffer and w == other.w and h == other.h and channels() ==
		// other.channels())
	...
	inline Image(int h, int w, Format f, uint8_t* buf) : w(w), h(h), format(f), buffer(buf), ownBuffer(false) {
		capacity = size();
	}

	// Size of scalar (uint8_t -> 1) (uint16_t -> 2), does not include channels.
	inline int32_t eleSize() const { return format == Image::Format::TERRAIN_2x8 ? 2 : 1; }
	// Size of entire image buffer
	inline int32_t size() const { return w * h * channels() * eleSize(); }
