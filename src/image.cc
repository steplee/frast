...
bool decode(Image& out, const EncodedImageRef& eimg) {
	if (out.format == Image::Format::TERRAIN_2x8) return decode_terrain_2x8(out, eimg);
	else return decode_jpeg(out, eimg);
}
bool encode(EncodedImage& out, const Image& img) {
	if (img.format == Image::Format::TERRAIN_2x8) return encode_terrain_2x8(out, img);
	else return encode_jpeg(out, img);
}
...
//   <--- Seperate these, clang-format should not re-order...
#include "detail/image_halfscale_impl.hpp"

void Image::warpAffine(Image& out, const float H[6]) const {
	if (out.format == Image::Format::RGBA and format == Image::Format::RGB) my_warpAffine<uint8_t, 4, 3>(out, *this, H);
	else {
...
void Image::halfscale(Image& out) const {
	if (out.format == Image::Format::GRAY) my_halfscale<uint8_t, 1, 1>(out, *this);
		...
		my_halfscale<uint8_t, 4, 4>(out, *this);
	else if (out.format == Image::Format::TERRAIN_2x8) my_halfscale<uint16_t, 1, 1>(out, *this);
	else
		throw std::runtime_error(std::string{"Image::halfscale() unsupported format/channels "} +
								 std::to_string(out.channels()));
}
...
void Image::remapRemap(Image& out, const float* map, int mapSizeW, int mapSizeH) const {
	...
	else if (out.format == Image::Format::RGBN or out.format == Image::Format::RGBA)
		my_remapRemap<uint8_t, 4>(out, *this, map, mapSizeW, mapSizeH);
	else if (out.format == Image::Format::TERRAIN_2x8) my_remapRemap<uint16_t, 1>(out, *this, map, mapSizeW, mapSizeH);
	else
		throw std::runtime_error(std::string{"Image::remapReamp() unsupported type/channels "} +
								 std::to_string(out.channels()));
}

template <int C>
static void makeGray_(Image& out, const Image& in) {
	int w = in.w, h = in.h;
	for (int y = 0; y < h; y++)
		for (int x = 0; x < w; x++) {
			uint16_t acc = 0;
...
void Image::makeGray(Image& out) const {
	if (format == Image::Format::TERRAIN_2x8) throw std::runtime_error("cannot make terrain grayscale.");
