...
AtomicTimer t_total("total"), t_encodeImage("encodeImage"), t_decodeImage("decodeImage"), t_mergeImage("mergeImage"),
...
Dataset::Dataset(const std::string& path, const DatabaseOptions& dopts, OpenMode m)
	: path(path), readOnly(m == OpenMode::READ_ONLY) {
	...
	if (auto err = mdb_env_open(env, path.c_str(), flags, fileMode)) {
		...
		// fmt::print( fmt::fg(fmt::color::red), " - [Dataset::Dataset()] WARNING: mdb_env_open on \"{}\" failed. If not testing, this is a problem.\n", path);
		failedOpen = true;
	}
	...
	if (not failedOpen) open_all_dbs();
}
...
bool Dataset::beginTxn(MDB_txn** txn, bool readOnly) const {
	...
	if (err) throw std::runtime_error(std::string{"(beginTxn) mdb_txn_begin failed with "} + mdb_strerror(err));
	return err != 0;
}
bool Dataset::endTxn(MDB_txn** txn, bool abort) const {
	AtomicTimerMeasurement g(t_dbEndTxn);
	if (abort) mdb_txn_abort(*txn);
	else mdb_txn_commit(*txn);
	*txn = nullptr;
	return false;
}
...
int Dataset::get_(MDB_val& val, const BlockCoordinate& coord, MDB_txn* txn) {
	...
	if (auto err = mdb_get(txn, dbs[coord.z()], &key, &val)) {
		// std::cout << " - mdb_get error: " << mdb_strerror(err) << " (block " << coord.z() << "z " << coord.y() << "y
		// " << coord.x() << "x)\n";
		return err;
	}
	return 0;
...
bool Dataset::getInflate_(Image& out, const BlockCoordinate& coord, MDB_txn* txn) {
	...
	// Sample the correct part of the image.
	{
		...
		*/
		...
		float H[9] = {// 1.f / div, 0, (float)x_off / div,
					  // 0, 1.f / div, (float)y_off / div,
					  (float)div, 0, -(float)x_off_pix * div, 0, (float)div, -(float)y_off_pix * div, 0, 0, 1.f};
...
int Dataset::put(const uint8_t* in, size_t len, const BlockCoordinate& coord, MDB_txn** givenTxn, bool allowOverwrite) {
	...
	if (err == MDB_KEYEXIST) {
		...
		*/
		...
		printf(" - mdb_put error: %s, (key %lu %lu %lu) (val %p, %zu)\n", mdb_strerror(err), coord.z(), coord.y(),
			   coord.x(), val.mv_data, val.mv_size);
	else
		// printf(" - mdb_put success: (key %lu %lu %lu) (val %p, %zu)\n", coord.z(), coord.y(), coord.x(), val.mv_data,
		// val.mv_size);
...
bool Dataset::erase(const BlockCoordinate& coord, MDB_txn* txn) {
	...
	if (auto err = mdb_del(txn, dbs[coord.z()], &key, nullptr)) return true;
...
uint64_t Dataset::determineLevelAABB(uint64_t tlbr[4], int lvl) const {
	...
	MDB_txn* txn;
	if (beginTxn(&txn, true)) throw std::runtime_error("Failed to open txn.");

	MDB_cursor* cursor;
	MDB_val		key, val;

	if (mdb_cursor_open(txn, dbs[lvl], &cursor)) throw std::runtime_error("Failed to open cursor.");
	...
	if (endTxn(&txn)) throw std::runtime_error("Failed to close txn.");
...
void Dataset::getExistingLevels(std::vector<int>& out) const {
	for (int lvl = 0; lvl < MAX_LVLS; lvl++)
		if (dbs[lvl] != INVALID_DB) out.push_back(lvl);
}
...
bool Dataset::hasLevel(int lvl) const { return dbs[lvl] != INVALID_DB; }
	...
	for (int i = 0; i < MAX_LVLS; i++) {
		if (meta.fixedSizeMeta.levelMetas[i].nTiles > 0)
			dprintf(" - (meta) lvl %2d : %6d tiles [%7d %7d -> %7d %7d]\n", i, meta.fixedSizeMeta.levelMetas[i].nTiles,
					meta.fixedSizeMeta.levelMetas[i].tlbr[0], meta.fixedSizeMeta.levelMetas[i].tlbr[1],
					meta.fixedSizeMeta.levelMetas[i].tlbr[2], meta.fixedSizeMeta.levelMetas[i].tlbr[3]);
	}
}
void Dataset::recompute_meta_and_write_slow(MDB_txn* txn) {
		...
		if (not hasLevel(lvl)) {
			...
			while (not curLvl.empty()) {
				...
				while (not st.empty()) {
					...
					// seen.insert(*it);
					curLvl.erase(it);
					bool hasParent = lastLvl.find(BlockCoordinate{ulvl + 1, coord.y() * 2 + 0, coord.x() * 2 + 0}.c) !=
										 lastLvl.end() or
									 lastLvl.find(BlockCoordinate{ulvl + 1, coord.y() * 2 + 1, coord.x() * 2 + 0}.c) !=
										 lastLvl.end() or
									 lastLvl.find(BlockCoordinate{ulvl + 1, coord.y() * 2 + 1, coord.x() * 2 + 1}.c) !=
										 lastLvl.end() or
									 lastLvl.find(BlockCoordinate{ulvl + 1, coord.y() * 2 + 0, coord.x() * 2 + 1}.c) !=
										 lastLvl.end();
					if (hasParent) {
				...
				if (nInRegion > 0) {
					regions.push_back({curRegion[0] * WebMercatorMapScale * 2. / (1 << ulvl) - WebMercatorMapScale,
					...
					printf(" - Found region (lvl %d) (%d tiles) (%lf %lf -> %lf %lf)\n", lvl, nInRegion,
						   regions.back().tlbr[0], regions.back().tlbr[1], regions.back().tlbr[2],
						   regions.back().tlbr[3]);
	...
	if (auto err = mdb_put(txn, metaDb, &key1, &val1, 0)) throw std::runtime_error("mdb_put error");
	if (auto err = mdb_put(txn, metaDb, &key2, &val2, 0)) throw std::runtime_error("mdb_put error");
...
void WritableTile::fillWith(const BlockCoordinate& c, const MDB_val& val) {
	AtomicTimerMeasurement g(t_tileBufferCopy);
	if (val.mv_size > eimg.size()) { eimg.resize(val.mv_size * 2); }
...
void DatasetWritable::w_loop() {
	...
	while (true) {
		...
		theCommand.cmd = DbCommand::NoCommand;
		{
			...
			if (!doStop and nWaitingCommands == 0) { dprintf(" - (wthread, spurious wakeup.\n"); }
		...
		if (theCommand.cmd != DbCommand::NoCommand) {
			if (theCommand.cmd == DbCommand::TileReady or theCommand.cmd == DbCommand::TileReadyOverwrite) {
				...
				// theCommand.data.tileBufferIdx, theWorker, theTileIdx, nWaitingCommands); fflush(stdout);
				WritableTile& tile = tileBuffers[theTileIdx];
				this->put(tile.eimg.data(), tile.eimg.size(), tile.coord, &w_txn,
						  theCommand.cmd == DbCommand::TileReadyOverwrite);
				curTransactionWriteCount++;
...
WritableTile& DatasetWritable::blockingGetTileBufferForThread(int thread) {
	...
	while (1) {
		...
		if (lendIdx - comtIdx < buffersPerWorker) {
			...
			nwaited++;
			if (thread == 0 and
				(nwaited == 20 or nwaited == 50 or nwaited == 100 or nwaited == 250 or nwaited % 500 == 0))
				printf(" - too many buffers lent [thr %d] (cmt %d, lnd %d, nbuf %d), waited %d times...\n", thread,
					   comtIdx, lendIdx, buffersPerWorker, nwaited);
			usleep(sleepTime);
...
void DatasetWritable::configure(int numWorkerThreads, int buffersPerWorker) {
	...
	for (int i = 0; i < nBuffers; i++) { tileBuffers[i].bufferIdx = i; }
...
void DatasetReader::loaderThreadLoop(int idx) {
	...
	} else std::cout << " - (DatasetReader::loaderThreadLoop()) opened  r_txn " << idx << " -> " << r_txns[idx] << "\n";
...
bool DatasetReader::rasterIoQuad(Image& out, const double quad[8]) {
	...
	const double quadColMajor[8] = {quad[0], quad[2], quad[4], quad[6], quad[1], quad[3], quad[5], quad[7]};
	double		 bboxWm[4]		 = {
					*std::min_element(quadColMajor + 0, quadColMajor + 4), *std::min_element(quadColMajor + 4, quadColMajor + 8),
					*std::max_element(quadColMajor + 0, quadColMajor + 4), *std::max_element(quadColMajor + 4, quadColMajor + 8)};

	double edgeLen;
	{
		// Heron's formula. This is a bit overkill.
		auto length_ = [](float x1, float y1, float x2, float y2) {
		...
		*/
		edgeLen = .5 * (length_(quad[0 * 2 + 0], quad[0 * 2 + 1], quad[1 * 2 + 0], quad[1 * 2 + 1]) +
	...
	if (lvl == BAD_LEVEL_CHOSEN) { return true; }
	...
	*/
	double sampledTlbr[4] = {((double)tileTlbr[0]) * WebMercatorMapScale * 2. / (1 << lvl) - WebMercatorMapScale,
	...
	alignas(16) float out_corners[8] = {0, 0, (float)ow, 0, (float)ow, (float)oh, 0, (float)oh};
	...
	for (int i = 0; i < 4; i++)
		...
		cv::line(dbgImg1, cv::Point{(int)in_corners[2 * i + 0], (int)in_corners[2 * i + 1]},
...
bool DatasetReader::rasterIo(Image& out, const double bboxWm[4]) {
	...
	assert(scale_x > 0.f);
	assert(scale_y > 0.f);
	assert(inset_tl_x <= 0.f);
	assert(inset_tl_y <= 0.f);
	float A[6] = {scale_x, 0, inset_tl_x, 0, scale_y, inset_tl_y};
	...
	float	  Ai[6] = {1.f / scale_x, 0, -inset_tl_x / scale_x, 0, 1.f / scale_y, -inset_tl_y / scale_y};
...
int DatasetReader::fetchBlocks(Image& out, uint64_t lvl, const uint64_t tlbr[4], MDB_txn* txn0) {
	...
	if (tlbr[0] == fetchedCacheBox[0] and tlbr[1] == fetchedCacheBox[1] and tlbr[2] == fetchedCacheBox[2] and
		tlbr[3] == fetchedCacheBox[3]) {
		// AtomicTimerMeasurement g(t_encodeImage);
		// printf(" - (fetchBlocks) cache hit copying (%d %d %d : %d) to (%d %d %d : %d).\n", fetchedCache.w,
		// fetchedCache.h, fetchedCache.channels(), fetchedCache.size(),
	...
	if (out.capacity < sw * sh * channels()) {
		...
		throw std::runtime_error(std::string{buf});
		// throw std::runtime_error("output buffer was too small for fetchBlocks (sw,sh " + std::to_string(sw) + "," +
		// std::to_string(sh) + ")");
	}
	out.w = sw;
	out.h = sh;
		...
		for (int yi = 0; yi < ny; yi++) {
			for (int xi = 0; xi < nx; xi++) {
				...
				if (out.format == Image::Format::TERRAIN_2x8) {
					uint16_t* dst = reinterpret_cast<uint16_t*>(out.buffer) +
									(ny - 1 - yi) * (tileSize() * sw * channels()) + xi * (tileSize() * channels());
					const uint16_t* src = reinterpret_cast<const uint16_t*>(accessCache1.buffer);
					if (channels() == 1)
						memcpyStridedOutputFlatInput<uint16_t, 1>(dst, src, sw, tileSize(), tileSize());
					else if (channels() == 3)
						memcpyStridedOutputFlatInput<uint16_t, 3>(dst, src, sw, tileSize(), tileSize());
					else if (channels() == 4)
						memcpyStridedOutputFlatInput<uint16_t, 4>(dst, src, sw, tileSize(), tileSize());
				} else {
					uint8_t* dst = (out.buffer) + (ny - 1 - yi) * (tileSize() * sw * out.channels()) +
								   xi * (tileSize() * out.channels());
					if (out.channels() == 4 and channels() == 1)
						memcpyStridedOutputFlatInputReplicateRgbPadAlpha<uint8_t>(dst, accessCache1.buffer, sw,
																				  tileSize(), tileSize());
					else if (out.channels() == 4 and channels() == 3)
						memcpyStridedOutputFlatInputPadAlpha<uint8_t>(dst, accessCache1.buffer, sw, tileSize(),
																	  tileSize());
					else if (channels() == 1)
						memcpyStridedOutputFlatInput<uint8_t, 1>(dst, accessCache1.buffer, sw, tileSize(), tileSize());
					else if (channels() == 3)
						memcpyStridedOutputFlatInput<uint8_t, 3>(dst, accessCache1.buffer, sw, tileSize(), tileSize());
					else if (channels() == 4)
						memcpyStridedOutputFlatInput<uint8_t, 4>(dst, accessCache1.buffer, sw, tileSize(), tileSize());
				}
			}
		}
	} else {
...
uint64_t DatasetReader::findBestLvlAndTlbr_dataDependent(uint64_t tlbr[4], uint32_t outCapacity, int imgH, int imgW,
														 const double bboxWm[4], MDB_txn* txn) {
	double boxW		   = bboxWm[2] - bboxWm[0];
	double boxH		   = bboxWm[3] - bboxWm[1];
...
uint64_t DatasetReader::findBestLvlAndTlbr_dataDependent(uint64_t tlbr[4], uint32_t outCapacity, int imgH, int imgW,
														 float edgeLen, const double bboxWm[4], MDB_txn* txn) {
	// 1. Find optimal level
	// 2. Find closest level that exists in entire db
	...
	if (lvl_ == BAD_FLOOR_VALUE) {
		...
		// fmt::print(fmt::fg(fmt::color::red), " - BAD FLOOR VALUE : Given bboxWm {}w {}h, edge {}, tileSize() {},
		// selecting level {}\n", bboxWm[2]-bboxWm[0],bboxWm[3]-bboxWm[1],edgeLen,tileSize(),lvl_);
	...
	// static constexpr int permOrder[MAX_LVLS] = { 10,11,12,13,14,15,16,17,18, 9,8,7,6,5,4, 19,20,21,22, 3,2,1,0,
	// 23,24,25 };
	...
	while (dbs[lvl] == INVALID_DB) {
		...
		if (lvl == -1) { dprintf(" - Failed to find valid db for lvl (originally selected %d)\n", lvl_); }
	}
	if (lvl_ != lvl) dprintf(" - Originally picked lvl %d, but not index there, so used lvl %lu\n", lvl_, lvl);
	lvl_ = lvl;
	...
	while (not good) {
...
#else
		uint64_t w =
			1 + std::max(std::ceil((WebMercatorMapScale + bboxWm[2]) * s) - (WebMercatorMapScale + bboxWm[0]) * s,
		...
		good = tileExists(BlockCoordinate{lvl, tlbr[1], tlbr[0]}, txn) and
			   tileExists(BlockCoordinate{lvl, tlbr[3] - 1, tlbr[2] - 1}, txn) and outCapacity >= sw * sh * channels();
		dprintf(" - Testing capacity %d vs %d\n", outCapacity, sw * sh * channels());

		// printf(" - does tile %luz %luy %lux exist? -> %s\n", lvl,tlbr[1],tlbr[0],
		// tileExists(BlockCoordinate{lvl,tlbr[1],tlbr[0]},txn) ? "yes" : "no"); printf(" - does tile %luz %luy %lux
		...
		if (lvl <= 0 or lvl >= 99) {
			printf(
				" - (findBestLvlAndTlbr) picked lvl %d, but searched all the way down to lvl 0 and could not find "
				"tiles to cover it.\n",
				lvl_);
			// throw std::runtime_error(std::string{"(findBestLvlAndTlbr) failed to find level with tiles covering
			// it"});
			return BAD_LEVEL_CHOSEN;
		}
	}
