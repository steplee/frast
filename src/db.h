...
constexpr uint32_t BAD_LEVEL_CHOSEN		  = 0xf00;
...
class Dataset {
  public:
	...
	Dataset(const std::string &path, const DatabaseOptions &dopts = DatabaseOptions{},
			OpenMode m = OpenMode::READ_ONLY);
	~Dataset();
	...
	bool get(std::vector<uint8_t> &out, const BlockCoordinate &coord,
			 MDB_txn **txn);  // By re-using output buffer, allocations will stop
	...
	template <class F>
	bool iterLevel(int lvl, MDB_txn *txn, F &&f) {
	...
	// This is a slow and naive function. Only call after a modifying app is
	// finished doing stuff on all levels.
  ...
  private:
	...
	// std::unique_ptr<AtomicTimerMeasurement> _t_total =
	// std::make_unique<AtomicTimerMeasurement>(t_total);
...
/*
 * I do not use move-semantics here because DatasetWritable holds the buffers,
 * and we want to avoid Image/vector re-allocations. Instead when the worker
 * (e.g. warper) threads wish to push a Tile to be stored, they ask for its
 * destination and the buffers are re-used if they are large enough (otherwise
 * e.g. the vector will be resized to be larger, but this will stop happening
 *  eventually)
 */
struct WritableTile {
	...
	inline WritableTile &operator=(WritableTile &&other) {
		copyFrom(other);
		return *this;
	}
...
/*
 * Simple type to help an app control the DatasetWritable writer thread
 * asynchronously.
 */
struct DbCommand {
	enum Type : int32_t { NoCommand, BeginLvl, EndLvl, EraseLvl, TileReady, TileReadyOverwrite } cmd = NoCommand;
	union Data {
		int32_t lvl;
 ...
 * This is because only one lonnnng write transaction is held the entire
 * duration.
 *
 * The Model:
 *         This has ONE writer thread that touches the LMDB database.
 *         A user app can have many threads writing asynchronously, but this
 * class contains just one internal thread that merges results from the worker
 * threads. The user app interacts with @DbCommand's, and via
 * @blockingGetTileBufferForThread() Typically, the user send BeginLvl, call
 * blockingGetTileBufferForThread() a bunch, send TileReady for every TileBuffer
 * receieved, then send EndLvl
 *
 *
 * You must also call sendCommand with StartLvl and EndLvl when starting/ending
 * a new pyramid level of writing.
 *
 *  This also has a tile cache like DatasetReader does.
 *  Unlike DatasetReader::tileCache, however, each cache should be *thread
 * local*. It is not safe to use from other threads, and obviously within the
 * current level being processed.
 *  TODO: barely tested the caching mechanism.
 *
 *  Note: This class uses mutexes, which may put the thread to sleep. I don't
 * *believe* this is bad here though. The writer thread should be encouraged to
 * yield cpu time to the workers. With a large enough @buffersPerWorker, the
 * writer should sleep long periods of time and wake and try to commit many
 * blocks at once. A bigger concern is also that the workers may sleep in
 * @blockingGetTileBufferForThread(). However, even then there should be enough
 * workers (and some of the image processing is multithreaded) that the CPUs
 * stay busy.
 *
 */
...
class DatasetWritable : public Dataset {
  ...
  private:
	...
	std::mutex				  w_mtx;
	std::vector<WritableTile> tileBuffers;	// We will have exactly numThreads *
											// buffersPerWorker elements.

	// Would be better to use one atomic int that stores the difference between
	// lended/commited, then does compare-and-swap, rather than locking.
	int		   tileBufferLendedIdx[MAX_THREADS];
	int		   tileBufferCommittedIdx[MAX_THREADS];
	std::mutex tileBufferIdxMtx[MAX_THREADS];

	// A worker pushes the index of the buffer to this list. It never grows larger
	// then N. If it would, sendCommand() blocks
 ...
 *			1) fetchBlocks(), caches the last output. If the same input
 *tlbr is passed, the last result is copied to the output buffer. 2) getCached()
 *has an LRU cache of READER_TILE_CACHE_SIZE entries.
 *
 * So fetchBlocks() (and the rasterIo* funcs) employ two-level caching.
 * Calls to getCached() that are new will incur a Dataset::get() call, which
 *also do a jpeg decode.
 *
 * fetchBlocks() is not thread safe.
 * getCached() is thread-safe, but only to internal threads. cacheMtx is
 *acquired if opts.nthreads > 1.
 *
 */
class DatasetReader : public Dataset {
  ...
  private:
	...
	Image				 accessCache;
	Image				 accessCache1;
	// TODO: have LruCache have a getView() method, then keep LruCache locked
	// until memcpyStridedOutputFlatInput is run, then unlock. This avoids a copy,
	// at the cost of a critical region (or not cost if nthreads=0)
	...
	//			1) The tiles needed do exist on that level, otherwise go up
	// until they do. 			2) The sample size is not too large for the output buffer
	//(passed as outCapacity)
	uint64_t findBestLvlForBoxAndRes(int imgH, int imgW, const double bboxWm[4]);
	uint64_t findBestLvlAndTlbr_dataDependent(uint64_t tileTlbr[4], uint32_t outCapacity, int imgH, int imgW,
