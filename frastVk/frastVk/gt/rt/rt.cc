...
template void GtRenderer<RtTypes, RtTypes::Renderer>::init(Device& d, TheDescriptorPool& dpool, SimpleRenderPass& pass, const AppConfig& cfg);
...
int RtDataLoader::loadTile(RtTile* tile, RtTypes::DecodedCpuTileData& dtd) {
	// Read file from disk
	// Parse protos
	...
	std::string fname = rootDir + "node/" + std::string{tile->coord.key, (size_t)tile->coord.level()};
	...
	if (dtd.metersPerTexel <= 0) {
		...
		tile->geoError = 1.0 / (1 << tile->coord.level());
...
void RtRenderer::initPipelinesAndDescriptors(TheDescriptorPool& dpool, SimpleRenderPass& srp, const AppConfig& cfg) {
		...
		gtpd.globalDescSet.create(dpool, gfxPipeline);
		...
		gtpd.tileDescSet.create(dpool, gfxPipeline);
		...
		for (int i=0; i<GT_NUM_TILES; i++) {
			...
			td.verts.set(2048*sizeof(PackedVertex),
					// VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_CACHED_BIT,
					VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
					VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
			td.inds.set(2048*sizeof(uint16_t),
					// VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_CACHED_BIT,
					VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
					VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
		...
		// GlobalData
		VkDescriptorBufferInfo bufferInfo { gtpd.globalBuffer, 0, 4*16 };
		gtpd.globalDescSet.update(VkWriteDescriptorSet{
				VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr,
				gtpd.globalDescSet, globalDataBindingIdx,
		...
		gtpd.tileDescSet.update(VkWriteDescriptorSet{
				VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr,
				gtpd.tileDescSet, globalDataBindingIdx,
