...
struct FtCoordinate : public BlockCoordinate {
	...
	// using BlockCoordinate::BlockCoordinate();
	// using BlockCoordinate::BlockCoordinate(uint64_t a);
	// using BlockCoordinate::BlockCoordinate(uint64_t z, uint64_t y, uint64_t x);

	// Deserialize
	// inline FtCoordinate(const std::string& a) : BlockCoordinate(*static_cast<const uint64_t*>(a.data())) {
	inline FtCoordinate(const std::string& a) : BlockCoordinate(*(const uint64_t*)(a.data())) {
	}

	/*
	inline FtCoordinate(const std::string& s) {
		int i =0;
		for (i=0; i<MAX_LEN; i++) {
...
struct FtTile : public GtTile<FtTypes, FtTile> {
	...
	bool upload(FtTypes::DecodedCpuTileData::MeshData& dctd, GtTileData& td);
